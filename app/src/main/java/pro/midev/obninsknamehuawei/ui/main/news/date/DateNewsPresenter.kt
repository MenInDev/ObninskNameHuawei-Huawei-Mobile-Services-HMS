...
import pro.midev.obninsknamehuawei.extensions.toServerDate
...
@InjectViewState
class DateNewsPresenter(
    private val date: Long?
) : BasePresenter<DateNewsView>() {
    ...
    override fun attachView(view: DateNewsView?) {
    ...
    fun loadNews(skip: Int, isPageLoading: Boolean) {
        service.getDateNews((date ?: Calendar.getInstance().timeInMillis).toServerDate(), skip)
            .map { it.toNewsHuman() }
